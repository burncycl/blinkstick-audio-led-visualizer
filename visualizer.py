...
class BlinkStickViz:
    def __init__(self, sensitivity, rate, chunk, channels, max_int, min_int, transmit, receive, inputonly, device=None):
        ...
        self.inputonly = False # Facilitates bypassing Blinkstick device, and handling input only device. Default to False.
        self.led_count = None # Determine the LED count by querying the stick(s). Populated by self.get_blinksticks() 
        if inputonly == False: # Facilitates bypassing Blinkstick device, and handling input only device.    
...
if __name__ == '__main__':
    ...
    elif args.inputonly == True:
        BlinkStickViz(sensitivity=args.sensitivity, rate=args.rate, chunk=args.chunk, channels=args.channels, max_int=args.max, min_int=args.min, transmit=args.transmit, receive=args.receive, inputonly=args.inputonly, device=args.dev).main()
    # Handle Receive mode.
    elif args.receive == True:
        BlinkStickViz(sensitivity=args.sensitivity, rate=args.rate, chunk=args.chunk, channels=args.channels, max_int=args.max, min_int=args.min, transmit=args.transmit, receive=args.receive, inputonly=args.inputonly, device=args.dev).udp_receive()
    # Handle Main
    elif args.modes is not None:
        BlinkStickViz(sensitivity=args.sensitivity, rate=args.rate, chunk=args.chunk, channels=args.channels, max_int=args.max, min_int=args.min, transmit=args.transmit, receive=args.receive, inputonly=args.inputonly, device=args.dev).main(modes=args.modes)
    else:
